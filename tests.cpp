...
TEST_CASE("testing peek() method", "[PEEK]") {
    ...
    SECTION("verify peek() returns correct value when peeking at non-empty stack") {
        s.push(1);
        s.push(2);
        INFO("peeking at non-empty stack should return 2");
        REQUIRE(s.peek() == 2);
        INFO("top pointer should be 1");
        REQUIRE(s.getTop() == 1);
    }
}
...
TEST_CASE("testing resize() method", "[RESIZE]") {
    IntArrayStack s;
    ...
    SECTION("verify stack capacity after pushing 16 integers, and then popping 12") {
        for (int i = 0; i < 12; i++) { s.pop(); }
        INFO("after pushing 16 integers and popping 12, the capacity should be 16");
        REQUIRE(s.getCapacity() == 8);
    }
